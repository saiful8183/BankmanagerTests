...
public class TwoTestBase {
	...
	public void setup(ITestContext context, String browserName) {
		...
		Capabilities capabilities = ((RemoteWebDriver) driver).getCapabilities();  // its an interface

		String device = capabilities.getBrowserName() + "_"+ capabilities.getBrowserVersion().substring(0, capabilities.getBrowserVersion().indexOf("."));
	...
	public void checkStatus(ITestResult result, ITestContext context, Method method) {
		...
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			String screenshotPath = captureScreenshot(context.getName() + "_" + method.getName());  // for attach screenshot in report
			extentTest.addScreenCaptureFromPath(screenshotPath);            // for attach screenshot in report
	...
	public String captureScreenshot(String name) {
		...
		File dstn = new File("ScreenShot/" + name + ".png");
		try {
			Files.copy(src, dstn);
